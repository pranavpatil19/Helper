...
/// </summary>
/// <remarks>
/// Used internally by the top-level query helpers to resolve mapper instances according to <see cref="RowMapperRequest"/>.
/// </remarks>
public sealed class RowMapperFactory : IRowMapperFactory
{
    ...
    /// <summary>
    /// Creates a factory that produces row mappers backed by the configured mapping profiles.
    /// </summary>
    /// <param name="options">Global mapper options (strategy defaults, caching, case handling).</param>
    /// <param name="profiles">Optional mapping profiles that adjust conversion/normalization logic.</param>
    /// <param name="columnNameNormalizer">Optional normalizer for providers that surface column names differently (e.g., Oracle uppercase).</param>
    ...
    /// <summary>
    /// Creates (or retrieves from cache) a strongly typed row mapper for the specified destination type.
    /// </summary>
    /// <typeparam name="T">Destination CLR type.</typeparam>
    /// <param name="request">Optional overrides such as strategy, case-sensitivity, or property-to-column map.</param>
    /// <returns>An <see cref="IRowMapper{T}"/> that can transform <see cref="System.Data.Common.DbDataReader"/> rows into <typeparamref name="T"/>.</returns>
