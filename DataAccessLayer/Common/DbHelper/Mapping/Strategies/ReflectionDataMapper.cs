...
/// <remarks>
/// <see cref="IRowMapperFactory"/> instantiates this mapper when <see cref="RowMapperRequest.Strategy"/> equals
/// <see cref="MapperStrategy.Reflection"/> (the default) so the top-level query helpers can hydrate strongly typed DTOs.
/// </remarks>
    ...
    where T : class
{
    ...
    /// <summary>
    /// Creates a mapper that projects <see cref="DbDataReader"/> rows into <typeparamref name="T"/> using reflection.
    /// </summary>
    /// <param name="ignoreCase">When true column/property name lookups are case-insensitive (recommended for ANSI providers).</param>
    /// <param name="propertyToColumnMap">Optional overrides mapping property names to explicit column names.</param>
    /// <param name="profiles">Optional profiles that can convert provider-specific values before assignment.</param>
    ...
    /// <summary>
    /// Materializes the next row from <paramref name="reader"/> into <typeparamref name="T"/>.
    /// </summary>
    /// <param name="reader">Source data reader positioned on the row to map.</param>
