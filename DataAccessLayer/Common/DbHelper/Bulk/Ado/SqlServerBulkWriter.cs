...
public sealed class SqlServerBulkWriter<T> : IBulkWriter<T>
{
    ...
    /// <summary>
    /// Executes the configured bulk copy synchronously using <see cref="SqlBulkCopy"/>.
    /// </summary>
    /// <param name="rows">Materialized rows to insert.</param>
    public void Write(IEnumerable<T> rows) =>
        WriteInternalAsync(rows, CancellationToken.None).GetAwaiter().GetResult();

    /// <summary>
    /// Executes the configured bulk copy asynchronously using <see cref="SqlBulkCopy"/>.
    /// </summary>
    /// <param name="rows">Materialized rows to insert.</param>
